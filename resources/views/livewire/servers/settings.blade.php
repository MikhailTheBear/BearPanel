<div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
    <div class="flex items-start justify-between gap-4 mb-6">
        <div>
            ...
            <h1 class="text-2xl font-semibold">{{ $server->name }} — {{ __('Settings') }}</h1>
    ...
    <div class="bg-white shadow rounded-lg p-6 space-y-5">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <div class="text-xs text-gray-500">{{ __('Container port') }}</div>
                <div class="font-mono text-sm">{{ $server->allocation_port ?? '—' }}</div>
            </div>

            <div>
                <div class="text-xs text-gray-500">{{ __('Host port') }}</div>
        ...
        <div>
            <label class="block text-sm text-gray-700">{{ __('Jar file name') }}</label>
        ...
        <div>
            <label class="block text-sm text-gray-700">{{ __('Java version') }}</label>
        ...
        <div>
            <label class="block text-sm text-gray-700">{{ __('Startup command template') }}</label>
            <textarea
