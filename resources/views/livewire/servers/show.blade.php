<div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8" wire:poll.2s="refreshServer">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <!-- Runtime -->
        <div class="bg-white shadow rounded-lg p-5">
            <h2 class="text-lg font-semibold mb-3">{{ __('Runtime') }}</h2>

            <div class="space-y-3 text-sm">
                <div>
                    <div class="text-gray-500">{{ __('Internal port') }}</div>
                    <div class="font-mono">{{ $server->allocation_port ?? '—' }}</div>
                </div>

                <div>
                    <div class="text-gray-500">{{ __('Container') }}</div>
                ...
                <div class="pt-2">
                    <button wire:click="recreate" class="w-full px-3 py-2 rounded-md border">
                        {{ __('Recreate container') }}
                    </button>
                    <div class="mt-2 text-xs text-gray-500">
                        {{ __('If container is broken or image changed.') }}
                    </div>
                </div>
            </div>
        </div>
